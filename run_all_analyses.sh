...
MASTER_ZIP="${CODE_DIR}/data/microprotein_master.zip"
...
if [ -f "$MASTER_CSV" ]; then
    ...
    echo "Please ensure the master data ZIP file (microprotein_master.zip) is available in Code/data/"
    exit 1
fi
...
cd "${CODE_DIR}/Codon_context"

if [ "$MODE" = "docs" ]; then
    print_docs "Sequence-context extraction (data processing - not run in summary mode)"
    echo "    Command: python build_sequence_context.py --espresso-prefix <prefix>"
    echo "    Status: Processing script available (requires the ESPRESSO sequence files)"
    echo ""
fi

# The initiation pipeline runs first: the Kozak pipeline uses its output as a regression
# baseline, checking that two independent implementations of the context score agree.
run_or_show "python initiation_pipeline.py" "non-ATG initiation-site analysis" "initiation_pipeline.py"
run_or_show "python kozak_pipeline.py" "start-codon Kozak context analysis" "kozak_pipeline.py"

# 1.3 RP3 Analysis Results
print_step "1.3 Running RP3 analysis summary..."
...
if [ "$MODE" = "docs" ]; then
    ...
    echo "    Scripts: convert_ESPRESSO_to_CPM.sh, convert_ESPRESSO_to_CPM_and_filter.py, deseq_brain_espresso.r"
...
if [ "$MODE" = "docs" ]; then
    ...
    echo "    Processing scripts: process_proteinID_from_TMT_round1.py, process_proteinID_from_TMT_round2.py, find_unique_tryptic_peptides.py"
    echo "    Matrix generation: generate_raw_TMT_intensity_matrix_across_batches.R, TMT_regressed_corrected_matrix.R"
    echo "    TAMPOR analysis: TAMPOR_round1.R, TAMPOR_round2.R, TAMPOR_combined_rounds.R"
...
# combined_gpath_results.csv is 84 MB — too large for GitHub, so it ships on
# Zenodo instead (large_data_tables.tar.gz). Without it this phase cannot run,
# but the rest of the pipeline is unaffected, so skip rather than abort.
if [ "$MODE" = "run" ] && [ ! -f "${CODE_DIR}/data/combined_gpath_results.csv" ]; then
    print_warning "Skipping scRNA-seq summary: Code/data/combined_gpath_results.csv not found."
    echo "    Download large_data_tables.tar.gz from https://doi.org/10.5281/zenodo.20045161"
    echo "    and extract combined_gpath_results.csv into Code/data/ to enable this phase."
    echo ""
else
...
# Add the scRNA-seq tab only if Phase 4 actually produced it
if [ -f "${RESULTS_DIR}/scRNA_Enrichment/scRNA_Enrichment_summary.csv" ]; then
    ...
    run_or_show "python generate_supplemental_tables.py" "Generate Supplemental Tables workbook (scRNA-seq tab omitted)" "generate_supplemental_tables.py"
fi
...
if [ "$MODE" = "docs" ]; then
    ...
    echo "    Results/Codon_context/kozak/kozak_context.csv"
    echo "    Results/Codon_context/initiation/initiation_summary.csv"
    ...
    echo "    Results/scRNA_Enrichment/scRNA_Enrichment_all_celltypes.csv.gz"
