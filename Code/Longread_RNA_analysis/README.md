...
└── ESPRESSO_data_processing_scripts/
    ├── convert_ESPRESSO_to_CPM.sh           # bash wrapper
...
1. **ESPRESSO -> CPM** (`convert_ESPRESSO_to_CPM_and_filter.py`,
   `convert_ESPRESSO_to_CPM.sh`): converts raw ESPRESSO counts to CPM and
...
bash ESPRESSO_data_processing_scripts/convert_ESPRESSO_to_CPM.sh

# deseq_brain_espresso.r resolves its inputs relative to the working directory
# (data/brain_espresso_medianCPM05.csv, data/nanopore_metadata.csv), so run it
# from Code/, not from this module directory.
(cd .. && Rscript Longread_RNA_analysis/ESPRESSO_data_processing_scripts/deseq_brain_espresso.r)
