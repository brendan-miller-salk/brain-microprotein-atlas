...
This module processes TMT (Tandem Mass Tag) proteomics data from FragPipe and performs quantitative analysis. The scripts in `fragpipe_results_processing_scripts/` are the same ones used to generate the summary data outputs here.
...
2. **`process_proteinID_from_TMT_round2.py`** - Process protein identifications from TMT round 2
...
5. **`TMT_regressed_corrected_matrix.R`** - Apply batch correction and normalization
...
#### FragPipe search configuration (`fragpipe/`):
The FragPipe runs that produced the inputs above: `fragpipe_round1.sh` and
`fragpipe_round2.sh` (launchers) with their `.workflow` definitions
(`fragpipe_workflow_round1.workflow`, `fragpipe_workflow_ms3_round1.workflow`,
`fragpipe_workflow_round2.workflow`). Kept for provenance — reproducing them
needs the raw mzML files, which are on PRIDE (PXD071500).
...
here are a pruned re-distribution of it. The SLURM wrapper is parameterized by
environment variable (see Usage below); the Python entry point still carries a
hardcoded `WORKSPACE` default pointing at the HPC layout.
...
```bash
# from the repo root
cd Code/Peptide_TMT_analysis
python Proteomics_Results_summary.py
```

Writes `Results/Proteomics/Proteomics_Results_summary.csv`.

### Run PROSIT validation:

`prosit_pipeline.py` is configured by environment variables, not command-line
paths — its `WORKSPACE` constant defaults to the analysis HPC layout. Point it at
your own FragPipe project through the SLURM wrapper, which reads
`PROSIT_WORKSPACE`, `PROSIT_INPUT_CSV`, and `PROSIT_OUTPUT_DIR`:

```bash
cd prosit
export PROSIT_WORKSPACE=/path/to/fragpipe_root
export PROSIT_INPUT_CSV=cleaned_tryptic_peptides_detailed_under_151aa.csv
export PROSIT_OUTPUT_DIR=prosit_out
sbatch run_prosit_pipeline.sh
```

Calling `prosit_pipeline.py` directly accepts only `--test`, `--phase1`,
`--phase2`, `--workers`, `--no-plots`, and `--annotate-only`; edit `WORKSPACE` at
the top of the file if you need to run it outside the wrapper.

## Dependencies
- Python: pandas, numpy, matplotlib, tqdm, pyteomics, scipy
- `koinapy`, for the PROSIT steps only. It is commented out in the repo's
  `requirements.txt` and `environment.yml`, so install it separately
  (`pip install koinapy`) before running the PROSIT pipeline.
...
## Outputs
- `../../Results/Proteomics/Proteomics_Results_summary.csv` — the summary table
  consumed by the dashboard and the manuscript figures
