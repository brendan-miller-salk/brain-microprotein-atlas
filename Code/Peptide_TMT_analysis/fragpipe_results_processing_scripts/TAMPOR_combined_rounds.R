...
# NORMALIZE RAW DATA
dat1 <- read.csv(file.path(TMT_DIR, 'shortstop_proteogenomics_appended_results/round1/C1.median_polish_corrected_relative_reporter_abundance_SHORTSTOP_AND_PROTEO_ROUND1.csv'))
dim(dat1)
# Set $X to rownames
...
dat2 <- read.csv(file.path(TMT_DIR, 'shortstop_proteogenomics_appended_results/round2/C1.median_polish_corrected_relative_reporter_abundance_SHORTSTOP_AND_PROTEO_ROUND2.csv'))
...
datadir <- file.path(TMT_DIR, "shortstop_results/")
numericMeta<-read.csv(file=file.path(TMT_DIR, "rosmap_metadata/combine_rounds_metadata.csv"),header=TRUE) %>%
  filter(dx != "GIS")
...
source(file.path(TMT_DIR, "normalization_scripts", "TAMPOR_custom.R"))
