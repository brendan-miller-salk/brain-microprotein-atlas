...
# NORMALIZE RAW DATA
dat <- read.csv(file.path(TMT_DIR, 'shortstop_proteogenomics_appended_results/round1/shortstop_proteogenomics_appended_raw_no_correction_intensity.csv'))
# Set $X to rownames
rownames(dat) <- dat$X
# Remove $X
dat <- dat[, -1]
...
datadir <- file.path(TMT_DIR, "shortstop_proteogenomics_appended_results/round1/")
numericMeta<-read.csv(file=file.path(TMT_DIR, "rosmap_metadata/rosmap_50batch_specimen_metadata_for_batch_correction.csv"),row.names=1,header=TRUE)
