...
# Author-local TMT proteomics working directory (controlled-access ROSMAP data;
# not shipped in the repo). Override with the TMT_DIR environment variable.
TMT_DIR <- Sys.getenv("TMT_DIR", "/Users/brendanmiller/Library/CloudStorage/Box-Box/brain_smorfs/tmt")

# NORMALIZE RAW DATA
dat <- read.csv(file.path(TMT_DIR, 'shortstop_proteogenomics_appended_results/round2/shortstop_proteogenomics_appended_raw_no_correction_intensity.csv'))
# Set $X to rownames
rownames(dat) <- dat$X
# Remove $X
dat <- dat[, -1]
...
datadir <- file.path(TMT_DIR, "shortstop_proteogenomics_appended_results/round2/")
numericMeta<-read.csv(file=file.path(TMT_DIR, "rosmap_metadata/rosmap_round2_specimen_metadata_for_batch_correction.csv"),row.names=1,header=TRUE)
...
source(file.path(TMT_DIR, "R_scripts", "TAMPOR_custom.R"))
TAMPORlist.GIShybrid<-TAMPOR(dat,numericMeta,noGIS=FALSE,useAllNonGIS=TRUE,GISchannels=c("126"),batchPrefixInSampleNames=TRUE,parallelThreads=8,outputSuffix="GIShybrid")
...
numericMeta<-numericMeta[-which(numericMeta$cogdx_3lev=="GIS"),]
...
write.csv(cleanRelAbun,paste0(outputtabs,"C1.median_polish_corrected_relative_reporter_abundance_SHORTSTOP_ROUND2.csv"))
write.csv(cleanDat,paste0(outputtabs,"C2.median_polish_corrected_log2abundanceRatioCenteredOnMedianOfBatchMediansPerProtein_SHORTSTOP_ROUND2.csv"))
