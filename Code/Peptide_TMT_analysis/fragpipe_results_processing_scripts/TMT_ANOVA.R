...
# Author-local input directories (controlled-access ROSMAP data + reference
# proteome; not shipped in the repo). Override with the TMT_DIR and UNIPROT_CSV
# environment variables.
TMT_DIR <- Sys.getenv("TMT_DIR", "/Users/brendanmiller/Library/CloudStorage/Box-Box/brain_smorfs/tmt")
UNIPROT_CSV <- Sys.getenv("UNIPROT_CSV", "/Users/brendanmiller/Library/CloudStorage/Box-Box/reference_genomes/human_genomes/uniprotkb_proteome_UP000005640_2024_12_09.csv")

# Read the normalized TMT intensity file
tmt <- read.csv(file.path(TMT_DIR, 'regressed_out_covariates_TMT_microproteins_shortstop_and_proteo_combined_rounds_PCAcorrected.csv'))

# Read the diagnosis file and select relevant columns
dx <- read.csv(file.path(TMT_DIR, 'rosmap_metadata/custom_dx_for_analyses.csv')) %>%
...
uniprot_sequences <- read.csv(UNIPROT_CSV) %>%
...
write.csv(ttest_results_uniprot, file.path(TMT_DIR, 'shortstop_proteogenomics_appended_results/ttest_control_vs_AD_tmt_shortstop_and_proteo_results_cpm05.csv'), row.names = F)
...
write.csv(anova_results, file.path(TMT_DIR, 'shortstop_proteogenomics_appended_results/anova_tmt_shortstop_and_proteo_results_cpm05.csv'), row.names = F)
write.csv(tukey_results, file.path(TMT_DIR, 'shortstop_proteogenomics_appended_results/tukey_tmt_shortstop__and_proteo_results_cpm05.csv'), row.names = F)
...
write.csv(transposed_df, file.path(TMT_DIR, 'shortstop_proteogenomics_appended_results/transposed_intensities_shortstop_and_proteo_percentiles_cpm05.csv'))
