...
# === Attach P-site frame stats (written by Psite_frame_mapping.py) ===
psite_file = os.path.join(outdir, 'Psite_frame_by_sequence.csv')
if os.path.exists(psite_file):
    psite = pd.read_csv(psite_file, usecols=['sequence', 'Psites_pct_frame0', 'Psites_pct_frame1',
                                             'Psites_pct_frame2', 'Psites_frame0_RPKM'])
    summary_salk = summary_salk.merge(psite, on='sequence', how='left')
