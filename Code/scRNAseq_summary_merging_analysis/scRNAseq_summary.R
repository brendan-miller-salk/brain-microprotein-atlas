...
# ------------------------------------------------------------------------------
# Export: every tested (microprotein, cell type) pair, unfiltered
# ------------------------------------------------------------------------------
# Everything downstream of here is filtered to p_adj.glb < 0.05, which makes a
# missing (microprotein, cell type) pair ambiguous: not significant, or never
# tested? The dashboard's per-microprotein cell-type panel needs to draw the
# non-significant cell types too, so write the pre-filter table with just the
# columns that panel plots. gzipped, since it is ~50x the rows of the filtered
# summary next to it.
gpath_annotated %>%
  ...
  # Stamp the p_adj.glb < 0.05 call BEFORE rounding, and let the dashboard read
  # this flag instead of re-deriving it. The largest passing q-value in the data
  # is 0.0499964, which rounds up to exactly 0.05 and would silently drop that
  # pair from the significant set the figures use.
  mutate(significant = `p_adj.glb` < 0.05) %>%
  select(
    sequence, gene, celltype, cell_type_general,
    logFC, logCPM, p_adj.glb, p_adj.loc, significant
  ) %>%
  # Display precision only. Full double precision triples the file for digits
  # no reader of the panel can use; the filtered summary next to it keeps the
  # unrounded values for anything analytical.
  mutate(
    logFC     = signif(logFC, 5),
    logCPM    = signif(logCPM, 5),
    `p_adj.glb` = signif(`p_adj.glb`, 4),
    `p_adj.loc` = signif(`p_adj.loc`, 4)
  ) %>%
  # Sorting by sequence is not cosmetic: each amino-acid sequence repeats once
  # per cell type, and grouping the copies adjacently is what lets gzip fold
  # them away (~9 MB scattered vs ~2 MB sorted). It is also the lookup order
  # the dashboard reads the table in.
  arrange(sequence, cell_type_general, celltype) %>%
  write_csv(file.path(OUT_DIR, "scRNA_Enrichment_all_celltypes.csv.gz"))
