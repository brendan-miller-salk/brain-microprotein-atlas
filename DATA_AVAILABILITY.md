...
([10.5281/zenodo.20045161](https://doi.org/10.5281/zenodo.20045161)) as seven
tarballs. **`./download_zenodo_assets.sh` fetches them, verifies each against
the checksum Zenodo publishes, and extracts it to the right path** — see
[Fetching the Zenodo assets](#fetching-the-zenodo-assets) below.
...
| `actin_micrographs.tar.gz`         | 25 downsampled PNG micrographs (`ev_*`, `psorf2_*`) — the inputs to the actin fluorescence quantitation in `Code/Miscellaneous/` |

### Fetching the Zenodo assets

```bash
./download_zenodo_assets.sh                 # list the archives, download nothing
./download_zenodo_assets.sh --all           # everything (~5.1 GB)
./download_zenodo_assets.sh large_data_tables large_reference_files
./download_zenodo_assets.sh --check         # re-verify what is already cached
```

The script downloads into `~/.cache/ad_microprotein_atlas/` (override with
`--cache DIR` or `$ZENODO_CACHE_DIR`), checks each tarball against the MD5
Zenodo publishes **and** against an expected entry count, and only then
extracts. A mismatch fails loudly and extracts nothing — v1.0.0 of the deposit
shipped two silently truncated archives, and this is the check that catches
that on the reader's side. Re-running skips anything already verified, so an
interrupted multi-GB pull resumes rather than restarts.

It pins the *version* record (`21245767`, v1.1) rather than the concept DOI, so
an older clone keeps getting the archives its code expects. Every extraction
path is already in `.gitignore`.

| Archive | Extracts to |
|---|---|
| `large_reference_files.tar.gz` | repo root -> `GTF_and_BED_files/`, `Results/RP3/` |
| `large_data_tables.tar.gz` | repo root -> `Code/data/` |
| `figures_mirror_plots.tar.gz` | `Results/mirror_plots/` |
| `figures_expression_profiles.tar.gz` | `Results/expression_profiles/` |
| `figures_smorf_cartoons.tar.gz` | `Results/smorf_cartoon_figures/` |
| `espresso_sequence_files.tar.gz` | `Code/Codon_context/espresso_sequences/` (pass this as `--espresso-prefix`) |
| `actin_micrographs.tar.gz` | `Code/Miscellaneous/` (pass this as `--image-folder`) |

`Code/data/microprotein_master_first_submission.csv` was in the v1.0.0
`large_data_tables.tar.gz` and does not carry forward; it stays retrievable
from [10.5281/zenodo.20045162](https://doi.org/10.5281/zenodo.20045162).
