...
COLUMN_DESCRIPTIONS = {
    ...
    'P-site % Frame 0': 'Percent of the ORF\'s P-sites at codon position 1 (in frame). Unreviewed smORFs matched by sequence, TrEMBL by CDS coordinates, Swiss-Prot by gene name + length.',
...
# Per-sequence P-site frame stats (Code/RP3_analysis/Psite_frame_mapping.py).
PSITE_FRAME_CSV = Path(__file__).resolve().parent / "RP3" / "Psite_frame_by_sequence.csv"
...
def _derive_rp3(mp):
    df = mp[mp["Database"].isin(["Salk", "Swiss-Prot-MP"])][[
    ...
    if PSITE_FRAME_CSV.exists():
        psite = pd.read_csv(PSITE_FRAME_CSV, usecols=["sequence"] + PSITE_FRAME_COLS)
        df = df.merge(psite, on="sequence", how="left")
    return df
...
# Evidence & Quality "≥N peptides" tiers (distinct tryptic peptide sequences).
# Nested like the significance tiers: ticking several is the same as ticking
# the lowest.
MIN_PEPTIDE_TIERS = [2, 3, 4]


def _count_peptides(pep_str):
    """Number of distinct tryptic peptide sequences in the master's list literal."""
    if not pep_str or pd.isna(pep_str):
        return 0
    try:
        peps = ast.literal_eval(str(pep_str))
    except (ValueError, SyntaxError):
        peps = [p for p in str(pep_str).split(';') if p.strip()]
    if isinstance(peps, str):
        peps = [peps]
    try:
        return len({str(p).strip() for p in peps if str(p).strip()})
    except TypeError:
        return 0


def _peptide_count_series(df):
    """`Tryptic_Peptide_Count` for `df`, derived on the fly if absent (same
    stale-cache fallback as _non_nterm_series)."""
    if 'Tryptic_Peptide_Count' in df.columns:
        return df['Tryptic_Peptide_Count'].fillna(0).astype(int)
...
def extract_unified_fields(master_df):
    ...
    ud['Tryptic_Peptide_Count'] = (
        ud['Tryptic_Peptides'].map(_count_peptides)
        if 'Tryptic_Peptides' in ud.columns
        else pd.Series(0, index=ud.index)
    )
    ...
    for src in PSITE_FRAME_COLS:
        cols = [c for c in master_df.columns if c.endswith(src)]
        if cols:
            ud[src] = pd.to_numeric(master_df[cols].bfill(axis=1).iloc[:, 0], errors='coerce')
...
def main():
    ...
    with st.sidebar:
        ...
        def _ribo_mask(df, lvl):
            if lvl == 'RiboCode-SAM':
                if 'RiboCode' not in df.columns or 'ShortStop_Label' not in df.columns:
                    return None
                return ((df['RiboCode'].astype(str).str.strip().str.upper() == 'TRUE')
                        & df['ShortStop_Label'].astype(str).str.startswith('SAM'))
            _pct = [f'Psites_pct_frame{i}' for i in range(3)]
            if not all(c in df.columns for c in _pct):
                return None
            return (df[_pct].apply(pd.to_numeric, errors='coerce') > 0).any(axis=1)

        _ribo_any = None
        for _lvl in selected_ribo:
            _m = _ribo_mask(base_df, _lvl)
            if _m is None:
                continue
            _ribo_any = _m if _ribo_any is None else (_ribo_any | _m)
        _add_mask('ribo', _ribo_any)
        ...
        with st.expander("Evidence & Quality", expanded=False):
            ...
            st.markdown("**Ribosome Coverage**")
            _rv = _narrow(skip='ribo')
            _ribo_help = {
            ...
            for _i, _n in enumerate(MIN_PEPTIDE_TIERS):
                _count_checkbox(f"f_min_peptides_{_n}", f"≥{_n} peptides",
                                int((_pc >= _n).sum()) if _pc is not None else 0,
                                help=("Keep microproteins identified by at least this many "
                                      "distinct tryptic peptide sequences (missed-cleavage "
    ...
    column_mapping = {
        ...
        'RP3_MM': 'RP3 MM',
        'RiboCode': 'RiboCode',
        'Psites_pct_frame0': 'P-site % Frame 0',
...
def _render_results_table(filtered_df, display_df):
    ...
    selection = st.dataframe(
        ...
        column_config={
            ...
            'P-site % Frame 0': st.column_config.NumberColumn('P-site % Frame 0', help='Percent of ORF P-sites in frame 0 (in-frame codon position)', format='%.1f'),
...
def _sec_riboseq(row, ctx):
    ...
    if any(_not_na(row.get(c)) for c in PSITE_FRAME_COLS):
        _kv_section('P-site Frame Distribution', [
            ('Frame 0 RPKM', _fmt(row.get('Psites_frame0_RPKM'), '%.3f')),
            ('% Frame 0', _fmt(row.get('Psites_pct_frame0'), '%.1f')),
