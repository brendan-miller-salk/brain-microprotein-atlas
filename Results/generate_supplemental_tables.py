...
def get_full_table_title(table_id):
    ...
    titles = {
        ...
        "S17": "Study Cohort Summary - Demographic and sample composition across all proteomic, transcriptomic, and ribosome-profiling cohorts (ROSMAP TMT, ROSMAP bulk RNA-seq, MSBB, UK Nanopore, Duffy Ribo-seq, UCSD DIA)",
...
def create_column_definitions_worksheet(writer):
    ...
    column_definitions = [
        ...
        ("S1", 'MS Evidence Type', 'DDA (ROSMAP TMT-MS, n = 610) or DIA (pilot UCSD samples, n = 4)'),
        ("S1", 'DDA Grade', 'Best PROSIT confidence tier for ROSMAP TMT-MS hits (Strong > Moderate > Weak > Insufficient) across tryptic peptides for DDA evidence quality.'),
        ("S1", 'PhyloCSF Score', 'PhyloCSF phylogenetic codon-substitution score; positive values indicate protein-coding evolutionary conservation.'),
        ...
        ("S2", 'TMT_log2fc_50pct_missing', 'Log2 fold-change of TMT intensity (AD vs. Control) — protein detected in at least 50% of donors (more stringent).'),
        ("S2", 'TMT_t_statistic_50pct_missing', 'Welch/Student t-statistic for the AD vs. Control TMT comparison — protein detected in at least 50% of donors.'),
        ("S2", 'TMT_df_50pct_missing', 'Degrees of freedom for the TMT t-test — protein detected in at least 50% of donors.'),
        ("S2", 'TMT_conf_low_50pct_missing', 'Lower bound of the 95% confidence interval on the mean log2 fold-change — protein detected in at least 50% of donors.'),
        ("S2", 'TMT_conf_high_50pct_missing', 'Upper bound of the 95% confidence interval on the mean log2 fold-change — protein detected in at least 50% of donors.'),
        ("S2", 'TMT_cohens_d_50pct_missing', "Cohen's d effect size for the AD vs. Control TMT comparison — protein detected in at least 50% of donors."),
        ("S2", 'TMT_pvalue_50pct_missing', 'Raw p-value for the TMT proteomics comparison — protein detected in at least 50% of donors.'),
        ("S2", 'TMT_qvalue_50pct_missing', 'BH-adjusted q-value for the TMT proteomics comparison — protein detected in at least 50% of donors.'),
        ("S2", 'TMT_log2fc_0pct_missing', 'Log2 fold-change of TMT intensity (AD vs. Control) — relaxed model; protein detected in at least one donor.'),
        ("S2", 'TMT_t_statistic_0pct_missing', 'Welch/Student t-statistic for the AD vs. Control TMT comparison — relaxed model; protein detected in at least one donor.'),
        ("S2", 'TMT_df_0pct_missing', 'Degrees of freedom for the TMT t-test — relaxed model; protein detected in at least one donor.'),
        ("S2", 'TMT_conf_low_0pct_missing', 'Lower bound of the 95% confidence interval on the mean log2 fold-change — relaxed model; protein detected in at least one donor.'),
        ("S2", 'TMT_conf_high_0pct_missing', 'Upper bound of the 95% confidence interval on the mean log2 fold-change — relaxed model; protein detected in at least one donor.'),
        ("S2", 'TMT_cohens_d_0pct_missing', "Cohen's d effect size for the AD vs. Control TMT comparison — relaxed model; protein detected in at least one donor."),
        ("S2", 'TMT_pvalue_0pct_missing', 'Raw p-value for the TMT proteomics comparison — relaxed model; protein detected in at least one donor.'),
        ("S2", 'TMT_qvalue_0pct_missing', 'BH-adjusted q-value for the TMT proteomics comparison — relaxed model; protein detected in at least one donor.'),
        ...
        ("S2", 'protein_class_length', 'Length of the protein class (Microprotein or Long protein greater than 150 aa).'),
        ...
        ("S2", 'mean_phylocsf', 'Mean PhyloCSF score across the ORF; positive values indicate protein-coding evolutionary conservation.'),
        ("S3", 'sequence', 'Amino acid sequence of the ORF product.'),
        ("S3", 'protein_id', 'Unique protein identifier from the mass spectrometry database.'),
        ("S3", 'peptide_sequence', 'List of tryptic peptide sequences identified for this protein.'),
        ("S3", 'start', 'List of start positions for each tryptic peptide within the protein sequence.'),
        ("S3", 'end', 'List of end positions for each tryptic peptide within the protein sequence.'),
        ("S3", 'is_unique_peptide', 'Boolean indicating whether the peptide(s) contain at least one unique to this protein.'),
        ...
        ("S4", 'RP3_MM_Amb', 'Both ambiguous and multi-mapping counts from RP3'),
        ...
        ("S4", 'total_razor_spectral_counts', 'Total razor peptide spectral counts assigned.'),
        ("S4", 'mean_phylocsf', 'Mean PhyloCSF score across the ORF; positive values indicate protein-coding evolutionary conservation.'),
        ("S5", 'CLICK_UCSC', 'Direct link to the genomic region in the UCSC Genome Browser for visualization.'),
        ("S5", 'Microprotein Sequence', 'Amino acid sequence of the microprotein (smORF translation product).'),
        ...
        ("S5", 'Annotation Status', 'Whether the microprotein was found in MS or RiboSeq/ShortStop.'),
        ("S5", 'PhyloCSF Score', 'PhyloCSF phylogenetic codon-substitution score; positive values indicate protein-coding evolutionary conservation.'),
        ("S6", 'Status', 'Whether the sequence is Reviewed (Swiss-Prot) or Unreviewed (TrEMBL and Novel; Salk-generated database or TrEMBL)'),
        ("S6", 'gene', 'Gene body expressed by cell type from PMID: 37774677'),
        ("S6", 'cluster_id', 'Cluster ID from PMID: 37774677'),
        ("S6", 'logFC', 'Fold change of gene expression counts from PMID: 37774677'),
        ("S6", 'logCPM', 'logCPM of gene expression counts from PMID: 37774677'),
        ("S6", 'F', 'F score from PMID: 37774677'),
        ("S6", 'p_val', 'Raw p value provided by PMID: 37774677'),
        ("S6", 'p_adj.loc', 'Adjusted p value provided by PMID: 37774677'),
        ("S6", 'p_adj.glb', 'Adjusted p value provided by PMID: 37774677'),
        ("S6", 'coef', 'Coefficient to pathology by PMID: 37774677'),
        ("S6", 'celltype', 'scRNA-Seq cell subtype annotations from PMID: 37774677'),
        ("S6", 'cell_type_general', 'scRNA-Seq cell type annotations from PMID: 37774677'),
        ("S6", 'gene_id', 'Ensembl gene identifier for the parent gene.'),
        ("S6", 'sequence', 'Amino acid sequence of the ORF product.'),
        ...
        ("S7", 'Status', 'Whether the sequence is Reviewed (Swiss-Prot) or Unreviewed (TrEMBL and Novel; Salk-generated database or TrEMBL)'),
        ("S7", 'CLICK_UCSC', 'Direct link to the genomic region in the UCSC Genome Browser for visualization.'),
        ("S7", 'sequence', 'Amino acid sequence of the ORF product.'),
        ("S7", 'gene_symbol', 'Standardized gene symbol for reviewed and unreviewed ORFs'),
        ("S7", 'genomic_coordinates', 'Genomic coordinates of the smORF (chr:start-end:strand).'),
        ("S7", 'rosmapRNA_baseMean', 'Mean normalized RNA expression across samples (ROSMAP bulk RNA-seq).'),
        ("S7", 'rosmapRNA_log2FoldChange', 'Log2 fold-change between disease vs. control (ROSMAP bulk RNA-seq).'),
        ("S7", 'rosmapRNA_pvalue', 'Raw p-value for differential expression (ROSMAP bulk RNA-seq).'),
        ("S7", 'rosmapRNA_padj', 'Benjamini–Hochberg adjusted p-value / FDR (ROSMAP bulk RNA-seq).'),
        ("S7", 'rosmapRNA_non_smorf_hit', 'Indicator of non-smORF overlapping signal (ROSMAP bulk RNA-seq).'),
        ("S7", 'rosmapRNA_body', 'Whether the entire gene body is differentially expressed or just the unique CDS of the gene body (ROSMAP).'),
        ("S7", 'ROSMAP_BulkRNAseq_CPM', 'Counts-per-million normalized RNA abundance (ROSMAP bulk RNA-seq).'),
        ...
        ("S7", 'msbbRNA_baseMean', 'Mean normalized RNA expression across samples (MSBB bulk RNA-seq).'),
        ("S7", 'msbbRNA_log2FoldChange', 'Log2 fold-change between disease vs. control (MSBB bulk RNA-seq).'),
        ("S7", 'msbbRNA_pvalue', 'Raw p-value for differential expression (MSBB bulk RNA-seq).'),
        ("S7", 'msbbRNA_padj', 'Benjamini–Hochberg adjusted p-value / FDR (MSBB bulk RNA-seq).'),
        ("S7", 'msbbRNA_non_smorf_hit', 'Indicator of non-smORF overlapping signal (MSBB bulk RNA-seq).'),
        ("S7", 'msbbRNA_body', 'Whether the entire gene body is differentially expressed or just the unique CDS of the gene body (MSBB).'),
        ("S7", 'MSBB_BulkRNAseq_CPM', 'Counts-per-million normalized RNA abundance (MSBB bulk RNA-seq).'),
        ("S7", 'Database', 'Database annotation source (e.g., Salk, Swiss-Prot, TrEMBL); Salk is an arbitrary name from custom GTFtoFASTA in silico translation tool'),
        ("S7", 'gene_name', 'Name of the parent gene name of the UNREVIEWED ORF gene; reviewed entries will be blank --> see gene_symbol'),
        ("S7", 'protein_length', 'Protein length in amino acids.'),
        ("S7", 'start_codon', 'ATG or nonATG'),
        ("S7", 'smorf_type', 'Type of smORF (uORF, dORF, intergenic, etc.).'),
        ("S7", 'total_razor_spectral_counts', 'Total razor peptide spectral counts assigned.'),
        ("S7", 'total_unique_spectral_counts', 'Total unique peptide spectral counts assigned.'),
        ("S7", 'mean_phylocsf', 'Mean PhyloCSF score across the ORF; positive values indicate protein-coding evolutionary conservation.'),
        ...
        ("S10", 'correlation', 'Correlation coefficient with the target transcript (Pearson R).'),
        ...
        ("S11", 'correlation', 'Correlation coefficient with the target transcript (Pearson R).'),
        ...
        ("S12", 'correlation', 'Correlation coefficient with the target transcript (Pearson R).'),
        ...
        ("S13", 'enst_id', 'Ensembl transcript identifier.'),
        ("S13", 'correlation', 'Correlation coefficient with the target transcript (Pearson R).'),
        ("S13", 'enst_clean', 'Cleaned version of Ensembl transcript ID for analysis.'),
        ("S13", 'external_gene_name', 'Gene name associated with the transcript.'),
        ("S13", 'abs_corr', 'Absolute value of the correlation coefficient.'),
        ...
        ("S16", 'Status', 'Whether the sequence is Reviewed (Swiss-Prot) or Unreviewed (TrEMBL and Novel; Salk-generated database or TrEMBL)'),
        ("S16", 'CLICK_UCSC', 'Direct link to the genomic region in the UCSC Genome Browser for visualization.'),
        ("S16", 'sequence', 'Amino acid sequence of the ORF product.'),
        ("S16", 'RP3_Default', 'Non ambiguous nor multi-mapping counts from RP3'),
        ("S16", 'RP3_MM_Amb', 'Both ambiguous and multi-mapping counts from RP3'),
        ("S16", 'RP3_Amb', 'Ambiguous counts from RP3'),
        ("S16", 'RP3_MM', 'Multi-mapping counts from RP3'),
        ("S16", 'RiboCode', 'RiboCode-based ORF translation call.'),
        ("S16", 'Database', 'Database annotation source (e.g., Salk, Swiss-Prot, TrEMBL); Salk is an arbitrary name from custom GTFtoFASTA in silico translation tool'),
        ("S16", 'gene_name', 'Name of the parent gene name of the UNREVIEWED ORF gene; reviewed entries will be blank --> see gene_symbol'),
        ("S16", 'gene_symbol', 'Standardized gene symbol for reviewed and unreviewed ORFs'),
        ("S16", 'protein_length', 'Protein length in amino acids.'),
        ("S16", 'start_codon', 'ATG or nonATG'),
        ("S16", 'smorf_type', 'Type of smORF (uORF, dORF, intergenic, etc.).'),
        ("S16", 'total_unique_spectral_counts', 'Total unique peptide spectral counts assigned.'),
        ("S16", 'total_razor_spectral_counts', 'Total razor peptide spectral counts assigned.'),
        ("S16", 'mean_phylocsf', 'Mean PhyloCSF score across the ORF; positive values indicate protein-coding evolutionary conservation.'),
        ("S17", 'Cohort / Dataset', 'Section header rows naming each cohort and assay (ROSMAP TMT Proteomics, ROSMAP Bulk RNA-seq, MSBB, UK Nanopore, Duffy Ribo-seq, UCSD DIA Proteomics) with a descriptive subtitle.'),
        ("S17", 'Experiment / Characteristic', 'Row label within each cohort block (e.g., TMT round, N, Age at death, Female/Male counts, or per-sample file).'),
        ("S17", 'Female, n (%)', 'Number and percentage of female participants in the group.'),
        ("S17", 'Male, n (%)', 'Number and percentage of male participants in the group.'),
        ("S17", 'Total N / N', 'Total number of participants or samples in the group.'),
        ("S17", 'Age at death, mean (SD)', 'Mean age (years) at death with standard deviation; diagnosis-stratified blocks report Control / AsymAD / AD / Total columns.'),
    ...
    # Order definitions by table number (S1 -> S18); stable sort preserves within-table order
    col_def_df['_sort'] = col_def_df['Table'].str.extract(r'(\d+)').astype(int)
    col_def_df = col_def_df.sort_values('_sort', kind='stable').drop(columns='_sort').reset_index(drop=True)
...
def create_toc_worksheet(writer):
    ...
    toc_data = {
        ...
        'Description': [
            ...
            'Demographic and sample composition across all cohorts used in the proteomic, transcriptomic, and ribosome-profiling analyses (ROSMAP TMT, ROSMAP bulk RNA-seq, MSBB, UK Nanopore, Duffy Ribo-seq, UCSD DIA)',
...
def main():
    ...
    # Path configuration.
    #   REPO_ROOT         -> this repo's Github/ folder (holds Results/, Code/).
    #   EXTERNAL_DATA_DIR -> author-local data not shipped in the repo (Box root);
    #                        override with the EXTERNAL_DATA_DIR env var.
    #   SUPPLEMENT_DIR    -> where the assembled workbook is copied; defaults to a
    #                        repo-relative supplementary/ folder, override via env.
    REPO_ROOT = Path(__file__).resolve().parent.parent
    EXTERNAL_DATA_DIR = Path(os.environ.get(
        "EXTERNAL_DATA_DIR",
    ...
    # Final destination for the assembled workbook.
    final_output_dir = Path(os.environ.get("SUPPLEMENT_DIR", REPO_ROOT / "supplementary"))
    ...
    # Define supplementary tables mapping. In-repo tables resolve under REPO_ROOT;
    # tables sourced from author-local data resolve under EXTERNAL_DATA_DIR.
    supplementary_tables = {
        "S1": {
            "path": str(REPO_ROOT / "Results/Annotations/Brain_Microproteins_Discovery_summary.csv"),
            "description": "Brain Microproteins Discovery Summary"
        },
        "S2": {
            "path": str(REPO_ROOT / "Results/Proteomics/Proteomics_Results_summary.csv"),
            "description": "Proteomics Results Summary"
        },
        "S3": {
            "path": str(REPO_ROOT / "Code/data/cleaned_tryptic_peptides_under_151aa.csv"),
            "description": "Tryptic Peptides Summary"
        },
        "S4": {
            "path": str(REPO_ROOT / "Results/RP3/RP3_Results_summary.csv"),
            "description": "RP3 Results Summary"
        },
        "S5": {
            "path": str(REPO_ROOT / "Results/Annotations/ShortStop_Microproteins_summary.csv"),
            "description": "ShortStop Microproteins Summary"
        },
        "S6": {
            "path": str(REPO_ROOT / "Results/scRNA_Enrichment/scRNA_Enrichment_summary.csv"),
            "description": "scRNA Enrichment Summary",
            "optional_scrna": True,
        },
        "S7": {
            "path": str(REPO_ROOT / "Results/Transcriptomics/Short-Read_Transcriptomics_Results_summary.csv"),
            "description": "Short-Read Transcriptomics Results"
        },
        "S8": {
            "path": str(REPO_ROOT / "Code/data/tss_motif_prevalence.csv"),
            "description": "TSS Motif Prevalence",
            # 75 MB — exceeds GitHub's limit, so it ships on Zenodo instead
            # (large_data_tables.tar.gz). Absent from a plain clone.
            "optional": True,
            "source": "Zenodo 10.5281/zenodo.20045161, large_data_tables.tar.gz",
        },
        "S9": {
            "path": str(REPO_ROOT / "Code/data/rbp_splice_per_junction.csv"),
            "description": "RBP Splice-Site Motifs"
        },
        "S10": {
            "path": str(EXTERNAL_DATA_DIR / "brain_smorfs/shortstop/deseq_on_rosmap/coexpression_outputs/target_ENST00000347364_7_chr20_10420549_10420737_F_2_P_3_M/coexpression_AD.csv"),
            "description": "Coexpression Analysis - AD (Target ENST00000347364)"
        },
        "S11": {
            "path": str(EXTERNAL_DATA_DIR / "brain_smorfs/shortstop/deseq_on_rosmap/coexpression_outputs/target_ENST00000347364_7_chr20_10420549_10420737_F_2_P_3_M/coexpression_nonAD.csv"),
            "description": "Coexpression Analysis - Non-AD (Target ENST00000347364)"
        },
        "S12": {
            "path": str(EXTERNAL_DATA_DIR / "brain_smorfs/shortstop/deseq_on_rosmap/coexpression_outputs/target_ENSG00000125863_20/coexpression_AD.csv"),
            "description": "Coexpression Analysis - AD (Target ENSG00000125863)"
        },
        "S13": {
            "path": str(EXTERNAL_DATA_DIR / "brain_smorfs/shortstop/deseq_on_rosmap/coexpression_outputs/target_ENSG00000125863_20/coexpression_nonAD.csv"),
            "description": "Coexpression Analysis - Non-AD (Target ENSG00000125863)"
        },
        "S14": {
            "path": str(EXTERNAL_DATA_DIR / "apex_omm/omm_differential_proteins.csv"),
            "description": "OMM Differential Proteins"
        },
        "S15": {
            "path": str(REPO_ROOT / "Results/Transcriptomics/Long-Read_Transcriptomics_Results_summary.csv"),
            "description": "Long-Read Transcriptomics Results"
        },
        "S16": {
            "path": str(REPO_ROOT / "Results/RP3/RP3_psORFs.csv"),
            "description": "RP3 psORFs"
        },
        "S17": {
            "path": str(EXTERNAL_DATA_DIR / "brain_smorfs/tmt/nature_aging_revisions/cohort_summary_table_combined.xlsx"),
            "description": "Study Cohort Summary"
        },
        "S18": {
            "path": str(REPO_ROOT / "Results/Annotations/smORF_type_definitions.csv"),
        ...
        for table_id, table_info in supplementary_tables.items():
            ...
            try:
                ...
                if not file_path.exists():
                    if table_info.get("optional"):
                        print(f"  ⏭️  SKIPPED: {file_path.name} not present locally")
                        skipped_tables.append(
                            (table_id, description, table_info.get("source", "")))
                        continue
                    print(f"  ❌ ERROR: File not found - {file_path}")
                    failed_tables.append((table_id, "File not found", str(file_path)))
                    continue
                
                # Load data (support both Excel and CSV source files)
                if file_path.suffix.lower() in ('.xlsx', '.xls'):
                    # Formatted multi-block report (e.g., S17 cohort summary):
                    # preserve the raw grid and suppress numeric column headers.
                    df = pd.read_excel(file_path, header=None)
                    df.columns = ['' for _ in range(df.shape[1])]
                else:
                ...
                if table_id == "S3":
                    try:
                        master_path = REPO_ROOT / "Code/data/microprotein_master.csv"
    ...
    if skipped_tables:
        print("⏭️  SKIPPED TABLES (optional inputs not present locally):")
        for table_id, desc, source in skipped_tables:
            print(f"  {table_id}: {desc}")
            if source:
                print(f"    Available from: {source}")
        print()
