...
from controlled-access individual-level data. Fetch them with
`./download_zenodo_assets.sh large_data_tables` (or by hand from
...
- Fetch with `./download_zenodo_assets.sh espresso_sequence_files` (~158 MB),
  which unpacks them into `Code/Codon_context/espresso_sequences/`; pass that
  directory's shared path prefix to `--espresso-prefix`. Extracting the tarball
  by hand anywhere else works too.

### Actin microscopy (`Code/Miscellaneous/`)
- 25 downsampled PNG micrographs (`ev_*`, `psorf2_*`), 52 MB — excluded from
  git for size.
- Fetch with `./download_zenodo_assets.sh actin_micrographs`, which unpacks the
  25 PNGs into `Code/Miscellaneous/`; pass that directory to `--image-folder`.
  Extracting the tarball by hand anywhere else works too.
- This module is standalone: it is not part of `run_all_analyses.sh` and
